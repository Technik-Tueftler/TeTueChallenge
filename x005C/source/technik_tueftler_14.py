...
def set_new_battleship(battle_field: list, ship_size: int, ship_sign: str) -> None:
    ...
    while running_condition:
        index = random.randint(0, len(fields_for_new_ship)-1)
        row, col = break_numbers(fields_for_new_ship[index])
        del fields_for_new_ship[index]
        for direction in directions:
            if next_field_valid(row, col, ship_size, direction, battle_field) is True:
                ...
                find_place = True
                break
        if len(fields_for_new_ship) <= 0 or find_place:
